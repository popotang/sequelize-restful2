...
Router.prototype.findDAOFactory = function(modelName) {
  return this.sequelize.daoFactoryManager.getDAO(modelName, {
    attribute: 'tableName'
  })
}
...
var getWhereCondition = function(daoFactory, identifier) {
  var condition = {
    where: {}
  };
  if (daoFactory.primaryKeyAttributes && daoFactory.primaryKeyAttributes[0]) {
    condition.where[daoFactory.primaryKeyAttributes[0]] = identifier;
  } else {
    condition.where["id"] = identifier;
  }
  return condition;
}
...
var handleResourceShow = function(modelName, identifier, callback) {
  ...
  if (!!daoFactory) {
    var condition = getWhereCondition(daoFactory,identifier);
    daoFactory
      .find(condition)
      .then(function(entry) {
        if (!entry) {
